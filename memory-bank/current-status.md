...
- Next.js App Router metadata and a system-font stack
- Client-only chart modules loaded with `next/dynamic`
- Next.js rewrite that forwards `/api` to FastAPI
...
- `vercel-react-best-practices` drove the React review and later Vite-to-Next.js migration. The App Router now owns metadata, while `next/dynamic` preserves client-only chart splitting and stable fallbacks.
...
- Next.js migration: 8 Vitest tests passed; ESLint and TypeScript passed; Next.js 16 production build passed.
- Lighthouse accessibility score before the framework migration: 100 on the rendered application.
